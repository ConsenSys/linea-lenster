...
export default async (network: 'mainnet' | 'testnet' | string, id: string) => {
  try {
    ...
    if (data.proposal) {
      return new Response(JSON.stringify({ success: true, spaceId: data.proposal.space.id }));
    }

    return new Response(JSON.stringify({ success: false, error: 'Invalid proposal id!' }));
  } catch (error) {
