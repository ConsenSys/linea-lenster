...
import { useAudio, useHuddle01, useLobby, useMeetingMachine, usePeers, useRoom } from '@huddle01/react/hooks';
...
const SpacePlayer: FC<SpacePlayerProps> = ({ publication, space }) => {
  ...
  const { fetchAudioStream, stopProducingAudio, produceAudio, stream: micStream } = useAudio();
      ...
      <div className="p-5">
        ...
        <div className="mt-2 space-y-3">
          ...
          {accessToken ? (
              ...
              <div>
                <div className="grid grid-cols-3 gap-6 sm:grid-cols-5">
                  {Object.values(peers)
                    ...
                    .map((peer) => (
                      <>
                        {peer.mic ? <Audio key={peer.peerId} peerId={peer.peerId} track={peer.mic} /> : null}
            ...
            <Button
              className="flex w-full justify-center"
              icon={signing ? <Spinner size="xs" className="mr-1" /> : <PencilAltIcon className="h-5 w-5" />}
              disabled={signing}
              onClick={async () => {
