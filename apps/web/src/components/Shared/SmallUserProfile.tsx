...
const SmallUserProfile: FC<UserProfileProps> = ({ profile, timestamp = '', smallAvatar = false }) => {
  const UserAvatar = () => (
    <Image
  ...
  const UserName = () => (
    <div className="flex max-w-sm items-center">
      <div className="truncate">{sanitizeDisplayName(profile?.name) ?? formatHandle(profile?.handle)}</div>
      {isVerified(profile?.id) && <BadgeCheckIcon className="text-brand ml-1 h-4 w-4" />}
      <Slug className="ml-2 text-sm" slug={formatHandle(profile?.handle)} prefix="@" />
      {timestamp ? (
