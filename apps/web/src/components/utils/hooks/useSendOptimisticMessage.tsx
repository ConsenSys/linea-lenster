...
import type { ContentTypeId, Conversation, DecodedMessage } from '@xmtp/xmtp-js';
...
export type PreparedMessage = Awaited<ReturnType<Conversation['prepareMessage']>>;
...
export type SendMessageContent<T extends AllowedContent = string> = T extends string ? T : () => Promise<T>;
...
const useSendOptimisticMessage = (conversationKey: string, options?: UseSendOptimisticMessageOptions) => {
  ...
  const sendMessage = async <T extends AllowedContent = string>(
    ...
    if (!sendOptions?.preparedMessage) {
      const preparedContent = typeof content === 'string' ? content : await content();
    ...
    try {
      ...
      if (pendingMessages.current.length > 0) {
        ...
        pending.resolve(
          await sendMessage(pending.message.content, pending.message.contentType, {
