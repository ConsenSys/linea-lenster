...
const Message: FC<MessageProps> = ({ conversationKey }) => {
  ...
  const queuedMessages = useMessageStore((state) => state.queuedMessages.get(conversationKey));
  const addQueuedMessage = useMessageStore((state) => state.addQueuedMessage);
  const removeQueuedMessage = useMessageStore((state) => state.removeQueuedMessage);
  const updateQueuedMessage = useMessageStore((state) => state.updateQueuedMessage);
  const [endTime, setEndTime] = useState<Map<string, Date>>(new Map());
  const { messages, hasMore } = useGetMessages(conversationKey, endTime.get(conversationKey));
  ...
  const { missingXmtpAuth, sendMessage } = useSendOptimisticMessage(conversationKey, {
  ...
  const allMessages = useMemo(() => {
    ...
    const finalQueuedMessage = (queuedMessages ?? []).reduce((result, queuedMessage) => {
