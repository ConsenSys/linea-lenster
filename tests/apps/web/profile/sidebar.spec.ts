...
test.describe('Profile sidebar', () => {
  ...
  test.describe('Profile badges', () => {
    ...
    test('should not have poh badge', async ({ page }) => {
      await expect(page.getByTestId('profile-poh-badge')).not.toBeVisible();
    });

    test('should not have sybil badge', async ({ page }) => {
      await expect(page.getByTestId('profile-sybil-badge')).not.toBeVisible();
    });

    test('should not have worldcoin badge', async ({ page }) => {
      await expect(
        page.getByTestId('profile-worldcoin-badge')
      ).not.toBeVisible();
    });
  });
});
